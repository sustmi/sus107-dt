...
void EmulatorView::init() {
	...
	wxString romName = wxT("48.rom");
	wxArrayString romPaths;
	romPaths.Add(wxT("."));
	romPaths.Add(standardPaths->GetUserDataDir());
	romPaths.Add(standardPaths->GetDataDir());

	wxString romDataPath(standardPaths->GetUserDataDir());
	romDataPath.Append(wxFileName::GetPathSeparator());
	romDataPath.Append(romName);

	// load ROM
	bool romOk = false;
	for (int i = 0; i < romPaths.Count(); i++) {
		wxString romFilename;
		romFilename.Append(romPaths[i]);
		romFilename.Append(wxFileName::GetPathSeparator());
		romFilename.Append(romName);
		romOk = emulator->getMachine()->loadRom(romFilename.mb_str().data());
		if (romOk) {
			break;
		}
	}
	...
	if (!romOk) {
		wxString tmpPaths;
		for (int i = 1; i < romPaths.Count(); i++) {
			tmpPaths.Append(wxT("\n"));
			tmpPaths.Append(romPaths[i]);
		}
		wxMessageBox(
			wxString::Format(_("Please place ROM file '%s' into current directory or one of these paths:\n%s"),
				romName.GetData(), tmpPaths.GetData()),
			wxString::Format(_("File %s not found"),
				romName.GetData())
		);
	}
